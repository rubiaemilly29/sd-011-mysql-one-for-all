...
AND(hist.usuario_id = (SELECT usuario_id FROM SpotifyClone.usuarios WHERE plano_id = 2)
OR hist.usuario_id = (SELECT usuario_id FROM SpotifyClone.usuarios WHERE plano_id = 3))
GROUP BY `nome`
ORDER BY `nome`;
