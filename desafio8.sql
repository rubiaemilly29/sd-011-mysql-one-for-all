...
FOR EACH ROW
BEGIN
DELETE FROM SpotifyClone.Seguidores AS seg 
WHERE seg.usuario_id = OLD.usuario_id;
DELETE FROM SpotifyClone.Historico AS hist
WHERE hist.usuario_id = OLD.usuario_id;
END $$
DELIMITER ;
